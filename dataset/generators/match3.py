"""
Match3 dataset generator.

Pure data generation: builds Match3 examples and serializes them to a JSON
Lines file (one example per line: {"seq": [...], "labels": [...]}). Has no
dependency on torch — reading the file back into a trainable form is the job
of `torch_datasets/match3.py`.

Task definition: each example is a sequence of integers drawn from
{1, ..., M-1}. A position is labeled 1 if it takes part in at least one triple
of positions (indices may repeat) whose values sum to 0 (mod M), and 0
otherwise. Sequences are generated and bucketed by their fraction of positive
labels so the dataset covers a spread of difficulties instead of skewing
toward mostly-negative sequences.
"""
...
def compute_labels(seq: np.ndarray, modulus: int) -> np.ndarray:
    ...
    achievable[np.unique(_pairwise_sums_mod(seq, modulus))] = True
...
def _split_candidates(
    ...
    if placed.size:
        ...
        achievable[np.unique(_pairwise_sums_mod(placed, modulus))] = True
...
def generate_sequence(
    ...
    for i in range(length):
        ...
        positive_cand, negative_cand = _split_candidates(placed, alphabet, modulus)
        ...
        if compute_labels(np.array(seq), modulus).sum() >= num_positive:
...
class Match3Generator(DatasetGenerator[Match3Example]):
    """
    Generates Match3 examples, balanced across `num_bins` buckets of
    positive-label percentage (0-100/num_bins%, ..., up to 100%).

    Implements the `DatasetGenerator` interface: `generate()` builds the
    examples, `to_record()` makes one JSON-serializable; `save()` (writing
    to JSON Lines) is inherited as-is.
    """
    ...
    def generate(self) -> List[Match3Example]:
        ...
        for _ in range(pool_size):
            ...
            seq = generate_sequence(length, self.alphabet, cfg.M, num_positive)
            labels = compute_labels(seq, cfg.M)  # ground-truth labels, independent of how seq was steered
